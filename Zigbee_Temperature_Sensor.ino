...
DHT_Unified dht(GPIO_NUM_14, DHT22);
#define PRESENCE_PIN GPIO_NUM_19
#define RGB_LED_PIN GPIO_NUM_8
Adafruit_NeoPixel rgbLed(1, RGB_LED_PIN, NEO_GRB + NEO_KHZ800);

/* Switch configuration */
#define GPIO_INPUT_IO_TOGGLE_SWITCH GPIO_NUM_9
#define GPIO_INPUT_RESET_SWITCH GPIO_NUM_19
...
static switch_func_pair_t button_func_pair[] = {{GPIO_INPUT_IO_TOGGLE_SWITCH, SWITCH_ONOFF_TOGGLE_CONTROL},{GPIO_INPUT_RESET_SWITCH, SWITCH_RESET_CONTROL}};
...
#define ESP_TEMP_SENSOR_MAX_VALUE       (60) /* Local sensor max measured value (degree Celsius) */
...
#define MANUFACTURER_NAME \
  "\x0B"                  \
  "Skye Harris"
#define MODEL_IDENTIFIER "\x09" "Temperature Sensor"
...
static void esp_app_humidity_sensor_handler(float humidity) {
  int16_t measured_value = zb_temperature_to_s16(humidity);
  Serial.println("Updating humidity sensor value...");
  Serial.println(measured_value);
  /* Update humidity sensor measured value */
  esp_zb_lock_acquire(portMAX_DELAY);

  esp_zb_zcl_set_attribute_val(
    HA_ESP_SENSOR_ENDPOINT, ESP_ZB_ZCL_CLUSTER_ID_REL_HUMIDITY_MEASUREMENT, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_REL_HUMIDITY_MEASUREMENT_VALUE_ID, &measured_value,
    false
  );
  ...
  switch (sig_type) {
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
      if (err_status == ESP_OK) {
        ...
        // Start Presence sensor reading task
        xTaskCreate(presence_sensor_value_update, "presence_sensor_update", 2048, NULL, 10, NULL);

        // Identify LED
        xTaskCreate(identify_task, "identify_task", 2048, NULL, 10, NULL);

          // Attach our attribute change handler
        esp_zb_identify_notify_handler_register(HA_ESP_SENSOR_ENDPOINT, &identifyHandler);
...
static esp_zb_cluster_list_t *custom_temperature_sensor_clusters_create(esp_zb_temperature_sensor_cfg_t *temperature_sensor) {
  ...
  esp_zb_occupancy_sensing_cluster_cfg_t occupancy_cfg = {
      ...
      .sensor_type_bitmap = 1,
  };

  // Humidity Sensor
  esp_zb_humidity_meas_cluster_cfg_t humidityConfig = {
    .measured_value = 0,
    .min_value = 0,
    .max_value = 100
  };
  ...
  ESP_ERROR_CHECK(esp_zb_cluster_list_add_humidity_meas_cluster(
    cluster_list, esp_zb_humidity_meas_cluster_create(&humidityConfig), ESP_ZB_ZCL_CLUSTER_SERVER_ROLE
  ));

  // Occupancy Sensing Cluster
  ESP_ERROR_CHECK(esp_zb_cluster_list_add_occupancy_sensing_cluster(
      cluster_list, esp_zb_occupancy_sensing_cluster_create(&occupancy_cfg), ESP_ZB_ZCL_CLUSTER_SERVER_ROLE
  ));
...
void identifyHandler(uint8_t identify_on)
{
  if (identify_on) {
    Serial.println("Identifying...");
  }

  identifyOn = identify_on;
}
...
static void temp_sensor_value_update(void *arg) {
  for (;;) {
    ...
    if (isnan(event.temperature)) {
      ...
      //esp_app_temp_sensor_handler(0);
    }
    else {
      Serial.print(F("Temperature: "));
      Serial.print(event.temperature);
      Serial.println(F("°C"));
      esp_app_temp_sensor_handler(event.temperature);
    }
    ...
    if (isnan(event.relative_humidity)) {
      ...
      Serial.println(F("%"));
      esp_app_humidity_sensor_handler(event.relative_humidity);
    }
...
/************************ Presence sensor *****************************/
static void presence_sensor_value_update(void *arg) {
  uint8_t lastPresence = 0;

  for (;;) {
    int8_t thisPresence = digitalRead(PRESENCE_PIN);

    if (thisPresence != lastPresence) {
      Serial.println("Updating presence sensor value...");
      Serial.println(thisPresence);
      
      esp_zb_lock_acquire(portMAX_DELAY);

      esp_zb_zcl_set_attribute_val(
        HA_ESP_SENSOR_ENDPOINT, ESP_ZB_ZCL_CLUSTER_ID_OCCUPANCY_SENSING, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_OCCUPANCY_SENSING_OCCUPANCY_ID, &thisPresence,
        false
      );

      esp_zb_lock_release();
      lastPresence = thisPresence;
    }
